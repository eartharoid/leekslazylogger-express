...
module.exports = class ExpressLogger extends Logger {
	constructor(options) {
		super(options);
		this.verbose(`[${this.timestamp()} | LOGGER] Express extension enabled`);
		if (!this.options.express) this.options.express = {};
		if (!this.options.express.format) this.options.express.format = '{method} {status-colour}{status} &7{path} {time-colour}({time})';
		
		this.express = (req, res, next) => {
			next(); // let the request continue

			const { method, protocol, route, baseUrl, path } = req;
